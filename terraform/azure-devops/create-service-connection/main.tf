...
locals {
  ...
  azdo_service_connection_name = "${replace(module.azure_access.subscription_name,"/ +/","-")}-${var.create_federation ? "oidc-" : ""}${var.create_managed_identity ? "msi" : "sp"}${terraform.workspace == "default" ? "" : format("-%s",terraform.workspace)}-${local.resource_suffix}"
